...
test('Transacciones Rechazadas', async ({ page }, testInfo) => {
    ...
    await allure.step('Navegar y acceder a Transacciones Rechazadas',async () => {
        ...
        //         page.click('a[href="/TransactionsRejected"]', { timeout: 60000 })
        //     ]);
        // });
    ...
    await allure.step('Verificar cantidad de transacciones rechazadas con fecha actual', async () => {
            ...
            await allure.step('Conteo de cantidad de transacciones rechazadas hoy', async () => {
                for (const row of rows) {
                const dateCell = await row.$('td:nth-child(5)'); 
            ...
            const mensaje = countToday > 10 
                ? `Hay más de 10 transacciones rechazadas hoy (${today})` 
                : `Cantidad de transacciones rechazadas hoy (${today}): ${countToday}`;

            await allure.step('Resultado de cantidad de transacciones rechazadas hoy', async () => {
