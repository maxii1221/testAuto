...
test('Reglas', async ({ page }, testInfo) => {
    ...
    allure.description(`
    Objetivo: Este test automatiza el proceso completo de creación, edición y eliminación de una regla.
    `);
    ...
    await allure.step('Crear Nueva Regla' , async () => {
        ...
        await allure.step('Desplegar y seleccionar compañia' , async () => {
            await page.locator('#IdCompanyContract').selectOption({ label: '001 - Debito' });
        });
